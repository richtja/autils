...
from autils import path
from tests.utils import skipUnlessPathExists
...
class Path(unittest.TestCase):
    def test_check_readable_exists(self):
        with unittest.mock.patch(
            "autils.path.os.path.exists", return_value=False
        ) as mocked_exists:
    ...
    def test_check_readable_access(self):
        with unittest.mock.patch(
            "autils.path.os.access", return_value=False
        ) as mocked_access:
            with self.assertRaises(OSError) as cm:
                path.check_readable(os.devnull)
            self.assertEqual(
    ...
    @unittest.mock.patch("autils.path.os.environ", {"PATH": ""})
    ...
    @unittest.mock.patch("autils.path.os.environ", {})
    @unittest.mock.patch("autils.path.os.path.isfile", return_value=True)
    @unittest.mock.patch("autils.path.os.access", return_value=False)
    ...
    def test_pathinspector_is_python_by_extension(self):
    ...
    def test_pathinspector_is_script_generic(self, mock_open, mock_isfile):
    ...
    def test_PathInspector_get_first_line(self, mock_open, mock_isfile):
    ...
    @unittest.mock.patch("autils.path.os.path.exists", return_value=True)
    @unittest.mock.patch("autils.path.os.stat")
    def test_PathInspector_has_exec_permission_exists(self, mock_stat, mock_exists):
    ...
    @unittest.mock.patch("autils.path.os.path.exists", return_value=True)
    @unittest.mock.patch("autils.path.os.stat")
    def test_PathInspector_is_empty(self, mock_stat, mock_exists):
        mock_stat_result = unittest.mock.MagicMock()
    ...
    def test_PathInspector_is_script_with_bash(self, mock_isfile, mock_open):
        inspector = path.PathInspector("test.sh")
    ...
    def test_PathInspector_is_script_no_shebang(self, mock_isfile, mock_open):
    ...
    def test_PathInspector_is_python_not_python(self, mock_isfile, mock_open):
        inspector = path.PathInspector("test.sh")
        self.assertFalse(inspector.is_python())

    @unittest.mock.patch("autils.path.os.path.isdir", return_value=False)
    @unittest.mock.patch("autils.path.os.makedirs", side_effect=OSError)
    def test_usable_rw_dir_create_fails(self, mock_makedirs, mock_isdir):
        self.assertFalse(path.usable_rw_dir("/tmp/test_dir"))

    @unittest.mock.patch("autils.path.os.path.isdir", return_value=True)
    @unittest.mock.patch("autils.path.tempfile.mkstemp", side_effect=OSError)
    def test_usable_rw_dir_fails(self, mock_mkstemp, mock_isdir):
        self.assertFalse(path.usable_rw_dir("/tmp"))

    @unittest.mock.patch("autils.path.os.path.isdir", return_value=True)
    @unittest.mock.patch("autils.path.os.close")
    @unittest.mock.patch("autils.path.os.unlink")
    @unittest.mock.patch(
        "autils.path.tempfile.mkstemp", return_value=(123, "/tmp/somefile")
    )
    ...
    @unittest.mock.patch("autils.path.os.path.isdir", return_value=False)
    @unittest.mock.patch("autils.path.os.makedirs")
    ...
    @unittest.mock.patch("autils.path.os.getcwd", side_effect=FileNotFoundError)
    def test_usable_ro_dir_getcwd_fails(self, mock_getcwd):
        self.assertFalse(path.usable_ro_dir("/tmp"))

    @unittest.mock.patch("autils.path.os.getcwd", return_value="/")
    @unittest.mock.patch("autils.path.os.path.isdir", return_value=True)
    @unittest.mock.patch("autils.path.os.chdir", side_effect=OSError)
    def test_usable_ro_dir_chdir_fails(self, mock_chdir, mock_isdir, mock_getcwd):
        self.assertFalse(path.usable_ro_dir("/tmp"))

    @unittest.mock.patch("autils.path.os.getcwd", return_value="/original_cwd")
    @unittest.mock.patch("autils.path.os.path.isdir", return_value=True)
    @unittest.mock.patch("autils.path.os.chdir")
    ...
    @unittest.mock.patch("autils.path.os.path.hasattr", return_value=True)
